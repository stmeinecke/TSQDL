...
// arma::mat get_g2mat(const std::vector<double> &timeseries1, const std::vector<double> &timeseries2)
// 	{	arma::mat g2mat;
// 		std::vector<double> g2;
// 		get_g2(timeseries1, g2);
// 		add_vec_to_matrix(g2mat, g2);
// 		get_g2(timeseries2, g2);
// 		add_vec_to_matrix(g2mat, g2);
// 		get_g2(timeseries1, timeseries2, g2);
// 		add_vec_to_matrix(g2mat, g2);
// 		g2.clear();
// 		return g2mat;
// 		
...
// 		std::cout << "Write file: "<< correlationoutname << std::endl;
		...
		for(int iter= -int(maxN/2); iter<int(maxN/2); iter+=every )  
			{corrout << iter*dt;
				for(int elem=0; elem<correlation[0].size(); elem++)
...
// void print_correlations(const arma::mat &correlation, Time dt, std::string correlationoutname, std::string correlationheader, std::size_t maxN, int every, unsigned int precision)
// 	{	std::ofstream corrout (correlationoutname, std::ios::out | std::ios::trunc);      //Output File (ofstream) overwrite
// 		corrout.precision(precision);
// 		size_t correlation_size= correlation.n_rows; //ARMA MAT ROWS
// 		corrout << correlationheader << std::endl; 
// 		if(maxN>correlation_size || maxN==0) maxN=correlation_size;		
// 
// 		for(int iter= -int(maxN/2); iter<int(maxN/2); iter+=every )  
// 			{corrout << iter*dt;
// 				for(int elem=0; elem<correlation.n_cols; elem++)
// 					{corrout << "\t" << correlation((iter+correlation_size)%correlation_size, elem);}
// 				corrout << std::endl;
// 			}					
// 		corrout.close();
...
//			{fourierout << iter/(powerspec.size()*dt) << "\t " << powerspec[iter%powerspec.size()]/powerspec[0] << std::endl;	}					
//
//		fourierout.close();
//	
//	}
...
//	}
//
//void dump_fourier_spec(std::vector<std::complex<double>> &spec, Time dt, int maxN, std::string fourieroutname)
//	{	std::ofstream fourierout (fourieroutname, std::ios::out | std::ios::trunc);      //Output File (ofstream) overwrite
//		fourierout <<"# Frequency [GHz] \t"  <<  "Fouriercomponent" << std::endl; 
//		if(spec.size()<maxN) maxN=spec.size();
//		for(int iter=(int) -maxN/2; iter<(int) maxN/2; ++iter )
